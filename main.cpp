...
void prescribePatient(int loggedInDocId) {
    ...
    std::ofstream outFile("prescriptions.txt", std::ios::app);
    if (outFile) {
...
// ================= SIMAR'S LAB DIAGNOSTIC FEATURES =================

void viewPendingLabOrders() {
    std::ifstream inFile("prescriptions.txt");
    std::cout << "\n==================================================\n";
    std::cout << "         ⚠️ PENDING DIAGNOSTIC LAB ORDERS          \n";
    std::cout << "==================================================\n";

    if (!inFile) {
        std::cout << "🟢 No active clinical prescription orders found on disk.\n";
        std::cout << "==================================================\n";
        return;
    }
    ...
    while (std::getline(inFile, line)) {
        ...
        if (!labTest.empty() && labTest != "None" && labTest != "none") {
            ordersFound = true;
            std::cout << "🧪 Patient ID: " << patId << " | Ordered Diagnostic: [" << labTest << "]\n";
        }
    }
    inFile.close();

    if (!ordersFound) {
        std::cout << "✅ All laboratory testing pipelines are currently clear.\n";
    }
    std::cout << "==================================================\n";
}
...
void submitLabReport() {
    int targetPatId;
    std::string testFindings;

    std::cout << "\n--- LABORATORY PROCESSING RADAR ---\n";
    std::cout << "Enter Patient ID to submit results for: ";
    std::cin >> targetPatId;
    std::cin.ignore();

    if (!displayPatientProfile(targetPatId)) {
        std::cout << "⚠️ Warning: Patient ID data card not tracked in intake database.\n";
    }

    std::cout << "Enter Lab Test Findings / Patient Biometrics: ";
    std::getline(std::cin, testFindings);

    std::ofstream outFile("lab_reports.txt", std::ios::app);
    if (outFile) {
        outFile << targetPatId << "," << testFindings << ",Complete\n";
        outFile.close();
        std::cout << "\n✔️ Lab Report generated and committed to lab_reports.txt successfully!\n";
    } else {
        std::cout << "❌ Error: Unable to write to lab records partition.\n";
    }
}
...
int main() {
    ...
    while (true) {
        std::cout << "\n==================================================\n";
        std::cout << "     CENTRAL HMS INTEGRATED MAIN MANAGEMENT CORE   \n";
        std::cout << "==================================================\n";
        std::cout << "1. Open Patient Intake Portal Subsystem\n";
        std::cout << "2. Open Doctor Clinical Portal Subsystem\n"; 
        std::cout << "3. Open Laboratory Diagnostics Subsystem (Simar's Portal)\n";
        std::cout << "4. Track Patient Booking Receipt Status\n";
        std::cout << "5. Exit Core Application Systems\n";
        
        int mainChoice = safeInputInt("Selection (1-5): ", 1, 5);
        ...
        if (mainChoice == 5) {
            ...
            bool insideLabPortal = true;
            while (insideLabPortal) {
                std::cout << "\n🧪 LABORATORY DIAGNOSTICS SUB-PORTAL CONTROL\n";
                std::cout << "1. View All Pending Lab Orders from Doctors\n";
                std::cout << "2. Input Test Results & Generate Lab Report\n";
                std::cout << "3. Log Out of Lab Subsystem\n";
                
                int labMenuChoice = safeInputInt("Selection (1-3): ", 1, 3);

                if (labMenuChoice == 1) {
                    viewPendingLabOrders();
                } else if (labMenuChoice == 2) {
                    submitLabReport();
                } else {
                    insideLabPortal = false;
                }
            }
        }
    }
